...
class JSON_API_Emend {
  ...
  function JSON_API_Emend($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    $this->avatar = get_avatar( $wp_comment, 40 );
    $this->content = strip_tags($content);
    $this->selection = html_entity_decode(get_comment_meta($wp_comment->comment_ID, 'emend_comment_selection', true));
  ...
  function comment_post_redirect() {
    ...
    $new_comment = new JSON_API_Emend($comment);
